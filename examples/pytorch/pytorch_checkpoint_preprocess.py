...
previous saved checkpoint using heartbeat.
    ...
    $ timeout 20 python examples/pytorch/pytorch_checkpoint_preprocess.py
    $ python examples/pytorch/pytorch_checkpoint_preprocess.py
"""
...
class Objective:
    ...
    def preprocess(self, study, trial):
        if "checkpoint_path" in trial.user_attrs:
            self.checkpoint = torch.load(trial.user_attrs["checkpoint_path"])
            self.epoch_begin = self.checkpoint["epoch"] + 1
        else:
            self.checkpoint = None
            self.epoch_begin = 0
    ...
    def __call__(self, trial):
        ...
        for epoch in range(self.epoch_begin, EPOCHS):
            model.train()
...
if __name__ == "__main__":
    ...
    objective = Objective()
    study.optimize(objective, n_trials=10, timeout=600, preprocess=[objective.preprocess])
